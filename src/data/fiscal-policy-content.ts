...
Look at what happens in round one. Government spends $10 billion building highways -- all $10 billion enters the economy immediately because the government is literally buying something from someone. Tax cut of $10 billion? That lands in household bank accounts, but if the MPC is 0.8, households save $2 billion right off the top. Only $8 billion actually enters the first spending round. The chain starts smaller and stays smaller.

The formula is **Tax multiplier = -MPC / (1 - MPC)**. Negative sign because taxes and AD move in opposite directions (a tax cut boosts AD, a tax hike shrinks it). With MPC = 0.8: -0.8 / 0.2 = **-4**. That $10 billion tax cut shifts AD right by $40 billion. Compare to $50 billion from equivalent direct spending. The gap between the spending multiplier and the tax multiplier (in absolute value) is always exactly 1, regardless of the MPC. Always. It comes from that first-round leakage, meaning government spending injects 100% immediately while the tax cut loses the savings fraction before anybody spends a dime.`,
    },
    {
...
When Congress spends more than it collects, the Treasury sells bonds to cover the gap. Those bonds compete for the same pool of loanable funds that businesses want for factory expansions, developers need for housing projects, families rely on for auto loans. More borrowers chasing a fixed pool of savings pushes **interest rates** up across the economy. A manufacturing firm that would have built a new plant at 4% shelves the project at 7%. A developer cancels a condo tower. A family decides the monthly car payment no longer works. Private investment drops, partially offsetting the government spending boost. This is the **crowding-out effect**, and it means the actual AD shift is smaller than the textbook multiplier would predict.

How severe is crowding out? Depends entirely on conditions. During the 2008 crisis, private investment demand had already collapsed and interest rates sat near zero -- there was almost nothing to crowd out, so government spending faced minimal displacement. Contrast that with an economy humming along at full employment where firms are actively competing for every dollar of savings. In that environment, government borrowing can push rates up substantially and squeeze private investment hard. The theoretical extreme is full crowding out, where every government dollar displaces exactly one private dollar and the net AD shift is zero. In practice, partial crowding out is the norm.`,
    },
    {
      heading: "Automatic Stabilizers",
      content: `Congress is slow. Committee hearings, floor debates, conference negotiations between House and Senate, presidential signature. The 2009 Recovery Act was considered fast and it still took nearly two months from inauguration day to signing. By the time money flows into the economy, the recession might have deepened or (awkwardly) already ended. **Automatic stabilizers** bypass all of that because they kick in without new legislation.
...
A deficit is a flow, meaning how much red ink in one fiscal year. The **national debt** is a stock, which is the total pile of outstanding IOUs at any moment. Stack up every deficit ever run, subtract the occasional surplus (1998-2001 being the most recent stretch), and that is your national debt.
...
Spending wins by $20 billion in total AD impact. On an AD/AS graph, both shift AD right but the spending option shifts it further. The gap is always exactly 1 between the multipliers (4 vs. 3 here) regardless of MPC.

Both policies raise real GDP and the price level as the economy moves up along the SRAS curve. The practical question boils down to: how much AD shift does Congress want per dollar of fiscal commitment?`,
    },
  ],
};
