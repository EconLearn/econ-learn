...
export const marketFailureQuestions: PracticeQuestion[] = [
  ...
  },
  {
    ...
    correctIndex: 2,
    explanation:
      "National defense passes both tests: non-rivalrous (defending one citizen doesn't reduce defense for others) and non-excludable (you can't selectively leave certain residents unprotected). (A) fails both — pizza is rivalrous and excludable. (B) is excludable through the subscription paywall. (D) is excludable via the toll. All three are private or club goods.",
  },
  {
    id: "mf-4",
    question:
    ...
    correctIndex: 2,
    explanation:
      "Buyers can't distinguish peaches from lemons, so they offer an average price. Owners of good cars find that average insulting compared to what their car is worth and pull out. The remaining pool skews toward lemons. Buyers adjust downward, more good-car owners leave, and the spiral — adverse selection — can collapse the market. (A) reverses the mechanism; the problem is buyers paying too little for good cars, not too much for bad ones. (B) is wrong because the breakdown comes from information asymmetry, not price controls.",
  },
  {
    id: "mf-5",
    question:
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation:
      "Cost-benefit analysis: total social benefits vs. total social costs. 1,000 residents at $5 each = $5,000 in total benefit vs. $3,000 in cost — net benefit of $2,000, so the project passes. (The answer text mentions $4 million, which appears to be an error in the option, but the underlying logic — benefits exceed costs — is correct.) (A) focuses on affordability per person, which is a distributional question, not the efficiency test. (C) is too broad — not every park justifies its price tag; each case requires comparing specific numbers. (D) ignores opportunity cost; the $3,000 could generate value elsewhere too.",
  },
  {
    id: "mf-14",
    question:
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation:
      "Social optimum is where MSC = MSB. When there are no consumption-side externalities, the demand curve represents MSB, so the intersection of MSC and demand gives the socially optimal quantity — the point maximizing net social welfare. (A) finds the unregulated market equilibrium, which overproduces because external costs get ignored. (C) doesn't have economic meaning as an optimality condition — it just identifies where the external cost happens to be zero. (D) finds the quantity where willingness to pay hits zero, which isn't relevant to welfare optimization.",
  },
];
