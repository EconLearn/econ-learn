...
export const elasticityContent = {
  title: "Elasticity",
  subtitle: "How much do buyers and sellers react when prices move?",
  sections: [
    {
      heading: "Price Elasticity of Demand",
      content: `If the price of gasoline jumps 20%, do drivers cut their consumption by 20%? By 5%? Not at all? **Price elasticity of demand (Ed)** answers this. It's a ratio that captures how sharply quantity demanded reacts to a price change:

**Ed = % change in quantity demanded / % change in price**

The raw coefficient comes out negative (demand curves slope downward), so economists work with the absolute value |Ed|. A common trap: assuming a straight-line demand curve has the same elasticity everywhere. It does not. The *slope* stays constant, but elasticity changes at every point because it depends on the ratio P/Q, not the slope alone.
...
For calculations, the AP exam expects the **midpoint method**, which avoids the problem of getting different answers depending on your starting point: %ΔQ = (Q2 - Q1) / ((Q1 + Q2)/2) and %ΔP = (P2 - P1) / ((P1 + P2)/2).`,
    },
    {
      ...
      content: `Compare a luxury handbag and insulin. Raise the price on both by 15%. Handbag sales might crater as customers switch to a competitor or skip the purchase. Insulin sales barely budge, because diabetic patients have no real alternative. That gap in responsiveness is the difference between **elastic** and **inelastic** demand.
...
|Ed| < 1 means **inelastic** demand. Quantity barely moves relative to the price change. Buyers absorb the hit. Gasoline, electricity, and prescription drugs fall in this category.
...
At the extremes: **perfectly elastic** demand (|Ed| = infinity) gives you a horizontal curve where even a tiny price increase sends quantity to zero. **Perfectly inelastic** demand (|Ed| = 0) gives a vertical curve where quantity stays fixed no matter what price does.`,
    },
    {
...
At **unit elasticity** (|Ed| = 1), revenue hits its peak. Move price in either direction and TR drops. The shaded rectangle on the graph represents TR at the selected price; drag the slider and watch the rectangle's area change.`,
    },
    {
      ...
      content: `Why is demand for one brand of cereal elastic while demand for electricity is not? Five factors do most of the work.
...
**Market definition** also changes things. Zoom out and demand looks inelastic: "food" is a necessity nobody can quit. Zoom in and it gets elastic fast: "organic avocados at the Whole Foods on 5th Street" has plenty of substitutes.`,
    },
    {
      ...
      content: `When Pepsi raises its price, what happens to Coke sales? When printer prices drop, what happens to ink cartridge sales? These questions require a different kind of elasticity, one that tracks how *two* goods relate.
...
The AP exam leans hard on sign and magnitude here. You should be comfortable reading a number like -0.6 or +2.3 and immediately classifying the good.`,
    },
    {
      ...
      content: `Elasticity shows up on roughly 15-20% of the AP Microeconomics exam, and the question patterns are predictable. Expect to:
...
Two mechanical skills get tested more than anything else: the **total revenue test** and the **midpoint formula**. Get the arithmetic right on those, and the reasoning clicks into place.`,
    },
    {
      heading: "Worked Example",
      content: `A local bookstore raises the price of a bestseller from **$4 to $6**. Weekly sales drop from **100 to 80 copies**. How elastic is demand?
...
export const elasticityQuestions: PracticeQuestion[] = [
  ...
  },
  {
    ...
    correctIndex: 2,
    explanation:
      "Slope stays constant on a linear demand curve, but elasticity does not — it depends on P/Q, which shifts at every point. At the midpoint, that ratio produces |Ed| = 1. Option A is wrong because the top of the curve (high P, low Q) gives |Ed| > 1, which is elastic, not unit elastic. Option B is wrong because the bottom (low P, high Q) gives |Ed| < 1, which is inelastic. Option D confuses constant slope with constant elasticity — a common mistake.",
  },
  {
    id: "elas-5",
    question:
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation:
      "Midpoint %ΔQ = (60 − 40) / 50 = 40%. Midpoint %ΔP = (8 − 12) / 10 = −40%. |Ed| = |40% / −40%| = 1.0 — unit elastic. Option A (0.5) comes from using simple percentage changes with the starting value as the base instead of the midpoint average. Option C (1.25) likely results from a calculation error in the denominator. Option D (2.0) would require quantity to change by twice the percentage of price, which doesn't match this data.",
  },
  {
    id: "elas-7",
    question:
