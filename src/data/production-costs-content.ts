...
export const productionCostsContent = {
  ...
  subtitle: "How firms transform inputs into outputs, and why cost curves are shaped the way they are",
  sections: [
    {
      heading: "The Short-Run Production Function",
      content: `How does a pizza shop decide how many workers to hire for a Friday night rush? It can't build a second kitchen by Friday. The ovens, counter space, and walk-in cooler are locked in. That constraint is what makes this the **short run**: at least one input (usually capital) is fixed, so the only lever is hiring more labor.
...
**Marginal product (MP)** measures the extra output from each additional worker. It *rises* early on because specialization kicks in, then *falls* once the fixed kitchen becomes a bottleneck. That rise-then-fall pattern drives everything else in this module.`,
    },
    {
      ...
      content: `A factory can't just keep hiring and produce infinite output. At some point, each new worker adds less than the one before. The **law of diminishing marginal returns** is a physical constraint, not a theory. Every short-run production process hits it.

The word *eventually* matters. A two-person assembly team outperforms two solo workers because they specialize: one wires, one solders. A third worker running quality checks boosts output further. Marginal product is *rising* during this phase. But pack twenty workers onto the same three-station line and they're tripping over each other, waiting for equipment, and slowing things down.

The direct link to costs: when the 8th worker adds 15 units of output and the 9th worker adds only 10, the cost of those last 10 units is higher per unit than the previous 15. Diminishing marginal returns *are* the reason **marginal cost (MC)** eventually climbs and why the MC curve has its characteristic U shape.`,
    },
    {
      heading: "From Production to Cost Curves",
      content: `The production function and cost curves are mirror images of each other. When each additional worker churns out *more* output (rising MP), the cost of each additional unit *drops* (falling MC). When MP falls, MC rises.
...
As output grows, AFC falls continuously because you're dividing a constant dollar amount by a bigger and bigger number. ATC and AVC squeeze closer together at high quantities because the shrinking AFC wedge between them approaches zero.`,
    },
    {
      ...
      content: `Your GPA works the same way. If this semester's grades (the "marginal") come in below your cumulative average, your cumulative GPA drops. If they come in above it, the cumulative rises. The marginal drags the average in its direction, and the crossover point is where the average *stops falling and starts rising*.

Cost curves follow the same math. While MC sits below ATC, every additional unit is cheaper than the current average, so ATC falls. Once MC climbs above ATC, each new unit is *more expensive* than the average, and ATC rises. **MC must cross ATC at its minimum point.** The same logic applies to AVC.
...
The **long-run average total cost (LRATC)** curve answers that question, and it has three distinct zones:
...
3. **Constant returns to scale** occupy the flat middle, where doubling all inputs doubles output and average cost holds steady. The quantity where LRATC first hits this floor is the **minimum efficient scale**. Industries where that number is enormous (chip fabrication, commercial aircraft) naturally end up with just a handful of giant firms.`,
    },
    {
      ...
      content: `Every output decision comes back to these cost curves. A competitive firm produces where P = MC. A monopolist produces where MR = MC. The MC curve is the backbone of the production decision regardless of market structure.

But deciding *how much* to produce is only half the question. The other half: should the firm produce *at all*? That depends on where price sits relative to ATC and AVC.
...
Stitching those rules together: a competitive firm supplies output along its MC curve, but only at prices above minimum AVC. Below that, quantity supplied is zero. The **short-run supply curve** is the segment of MC that sits above AVC.`,
    },
    {
...
Strip out fixed costs to find variable costs: TVC = TC - TFC = $350 - $100 = $250. That $250 covers ink, blank shirts, and labor, costs that only exist because the shop produced something.
