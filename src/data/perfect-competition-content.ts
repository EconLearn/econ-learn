...
export const perfectCompetitionContent = {
  ...
  subtitle: "How price-taking firms maximize profit in markets with free entry and exit",
  sections: [
    {
...
That is what it means to be a **price taker**. The firm faces a horizontal demand curve at the market price. Sell as much as you want at that price, but try to charge even a penny more and your sales drop to zero.
...
Four conditions create this situation: many small firms, identical products, perfect information, and free entry/exit. No single real-world market hits all four perfectly, but agricultural commodities and certain financial markets come close enough that the model generates accurate predictions.`,
    },
    {
      ...
      content: `How does a wheat farmer decide how many acres to plant? The same rule every firm uses: keep producing as long as each additional unit brings in more revenue than it costs.

In competitive markets, selling one more bushel adds exactly the market price to revenue. So **MR = P**, and the profit-maximizing rule **MR = MC** simplifies to **P = MC**. Find where the market price intersects the marginal cost curve. That is the optimal quantity.
...
Try it on the graph: shift market demand or supply and watch how the equilibrium price changes. The firm's output adjusts automatically to wherever P = MC lands on the cost curve.`,
    },
    {
      ...
      content: `Three scenarios, depending on where the market price lands relative to the firm's cost curves.
...
**P < AVC**: time to close. Revenue doesn't even cover variable costs, so every unit produced digs the hole deeper. The firm produces nothing and eats the fixed costs. That is the **shutdown point**.`,
    },
    {
      heading: "The Shutdown Condition",
      content: `This shows up on AP exams constantly, so get the logic straight.
...
The rule: **shut down when P < minimum AVC**. The firm's **short-run supply curve** is the portion of MC that lies above that minimum AVC threshold. Below it, quantity supplied is zero.`,
    },
    {
      ...
      content: `Profits attract competitors. Losses drive them away. Those two sentences explain the entire long-run adjustment process.
...
The endpoint is **P = minimum ATC** for every firm. Economic profit is zero. That does not mean the business is failing. It means owners are earning exactly what they'd earn in their best alternative investment. Zero *economic* profit still includes a normal accounting return. The firm is fine; it's just not earning anything above what it takes to keep capital invested.`,
    },
    {
...
**Allocative efficiency** means P = MC. The price of the last unit sold equals what it cost to produce. Resources go exactly where consumers value them most. You cannot reshuffle production to make someone better off without making someone else worse off.
...
export const perfectCompetitionQuestions: PracticeQuestion[] = [
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation:
      "In perfect competition, the firm is a price taker. The demand curve facing the individual firm is perfectly elastic (horizontal) at the market price. Since every unit sells for $7, average revenue (TR/Q) = $7 and marginal revenue (additional revenue from one more unit) = $7. All three curves are the same horizontal line at $7. (A) describes a monopolist or monopolistically competitive firm that must lower price to sell more — this does not apply to a price taker. (C) is wrong because MR only slopes downward when the firm faces a downward-sloping demand curve (monopoly); here the firm sells every unit at the same price, so MR is constant. (D) vertical curves would mean quantity is fixed regardless of price, which describes perfectly inelastic supply or demand — not relevant to a competitive firm's revenue curves.",
  },
  {
    id: "perf-12",
    question:
    ...
    correctIndex: 0,
    explanation:
      "Economic profit per unit = P − ATC = $20 − $15 = $5. Total economic profit = profit per unit × quantity = $5 × 500 = $2,500. (B) incorrectly subtracts AVC ($11) from price ($20) to get $9, but that gives the per-unit contribution margin over variable costs, not economic profit — profit must be measured against ATC which includes both variable and fixed costs. (C) uses the full price of $20 as profit, ignoring costs entirely. (D) assumes zero profit, but the firm's P ($20) exceeds ATC ($15), so it is earning positive economic profit — long-run equilibrium has not yet been reached because entry hasn't driven price down to minimum ATC.",
  },
  {
    id: "perf-13",
    question:
